do		
 local Executive1 = 156873997 --put your id here(BOT OWNER ID)
 		
 local function setrank(msg, name, value) -- setrank function		
   local hash = nil		
   if msg.to.type == 'chat' then		
     hash = 'ًں”¹Rankًں‘‘ :'..msg.to.id..':variables'		
   end		
   if hash then		
  ...
  local function res_user_callback(extra, success, result) -- /info <username> function		
   ...
   if result.username then		
    ...
    Username = '----'		
     ...
     local text = 'ًں”¹Full nameًں‘پ : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'		
                ..'ًں”¹Usernameâ­•ï¸ڈ : '..Username..'\n'		
                ..'ًں”¹ID â­•ï¸ڈ:  '..result.id..'\n'		
  	local hash = 'ًں”¹Rankًں‘‘ :'..extra.chat2..':variables'		
  	local value = redis:hget(hash, result.id)		
     if not value then		
  	   if is_admin2(result.id) then		
  	   text = text..'ًں”¹Rankًں‘‘ : Admin \n'		
    elseif is_owner2(result.id, extra.chat2) then		
    text = text..'ًں”¹Rankًں‘‘ : Owner \n'		
   elseif is_momod2(result.id, extra.chat2) then		
    text = text..'ًں”¹Rankًں‘‘ : Moderator \n'		
       else		
   text = text..'ًں”¹Rankًں‘‘ : : Member \n'		
  end		
    else		
    text = text..'ًں”¹Rankًں‘‘ : '..value..'\n'		
   end		
  if result.from.id == tonumber(Executive1) then		
       text = text..'ًں”¹Sudo ًں”±: Yes \n'		
 		     ...
 		     elseif result.from.id == tonumber(Executive5) then		
 		       ...
 		       text = text..'ًں”¹Sudo ًں”±: No \n'		
 		 end		
   local uhash = 'user:'..result.id		
   local user = redis:hgetall(uhash)		
   local um_hash = 'msgs:'..result.id..':'..extra.chat2		
   user_info_msgs = tonumber(redis:get(um_hash) or 0)		
   text = text..'ًں”¹messages sent ًں’¬: '..user_info_msgs..'\n'		
   text = text..'....'		
 ...
 local function action_by_reply(extra, success, result)-- (reply) /info  function		
   ...
   local text = 'ًں”¹Full nameًں‘پ : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'		
                ..'ًں”¹Usernameâ­•ï¸ڈ : '..Username..'\n'		
                ..'ًں”¹ID â­•ï¸ڈ:  '..result.from.id..'\n'		
 	local hash = 'ًں”¹Rankًں‘‘ '..result.to.id..':variables'		
 ...
 local function run(msg, matches)		
  ...
  if matches[1]:lower() == 'info' and not matches[2] then		
   ...
   if msg.from.username then		
    ...
    local text = 'ًں”¹First nameًں‘پ : '..(msg.from.first_name or '----')..'\n'		
    local text = text..'ًں”¹Last nameًں‘€ : '..(msg.from.last_name or '----')..'\n'			
    local text = text..'ًں”¹Usernameâ­•ï¸ڈ : '..Username..'\n'		
    local text = text..'ًں”¹ID â­•ï¸ڈ:  '..msg.from.id..'\n'		
    local hash = 'ًں”¹Rankًں‘‘'..msg.to.id..':variables'		
 	if hash then		
 	 ...
 	 text = text..'ًں”¹messages sent ًں’¬:  '..user_info_msgs..'\n'		
     if msg.to.type == 'chat' then		
 	 text = text..'ًں”¹Group name ًں‘¥: '..msg.to.title..'\n'		
      text = text..'ًں”¹Group ID ًں‘¥:  '..msg.to.id		
     end		
     text = text..'\n\n@zsgroupTG'		
